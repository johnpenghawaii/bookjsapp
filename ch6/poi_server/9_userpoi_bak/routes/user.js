  ...
  .post(function(req, res) {
    ...
    saveIfNewEmail(user, function(err, result) {
      if (!err) {
        ...
        res.end(report('OK', ret));
      } else {
        console.error(err);
        res.end(report(err.code));
      }
    });
  })
...
router.route('/:id')
  ...
  .put(function(req, res) {
    ...
    upsert(data, function(err, result) {
      if (!err) res.end(report('OK', ret));
      else res.end(report(err.code));
    });
  })
  .delete(function(req, res) {
    var id = req.params.id;
    User.remove({_id: id}, function(err, result) {
      if (!err && result === 1) res.end(report('OK'));
      else if (result === 0) {
        res.end(report('ID_MISSING'));
      }
      else {
        console.error(err);
        res.end(report(err.code));
